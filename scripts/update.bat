...
%MYSQL% -u %USER% -p%PASS% acore_characters < "%SQL_CHARS%\item_affix.sql"
if %ERRORLEVEL% neq 0 ( echo ERROR: item_affix.sql failed & pause & exit /b 1 )
%MYSQL% -u %USER% -p%PASS% acore_characters < "%SQL_CHARS%\item_talent_affix.sql"
...
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\affix_template.sql"
if %ERRORLEVEL% neq 0 ( echo ERROR: affix_template.sql failed & pause & exit /b 1 )
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\talent_affix_def.sql"
if %ERRORLEVEL% neq 0 ( echo ERROR: talent_affix_def.sql failed & pause & exit /b 1 )

REM Imprint system
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\imprint_def.sql"
if %ERRORLEVEL% neq 0 ( echo ERROR: imprint_def.sql failed & pause & exit /b 1 )
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\imprint_rune_items.sql"
if %ERRORLEVEL% neq 0 ( echo ERROR: imprint_rune_items.sql failed & pause & exit /b 1 )
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\spell_script_names_imprint.sql"
...
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\spell_dbc_celestial_resonance.sql"
if %ERRORLEVEL% neq 0 ( echo ERROR: spell_dbc_celestial_resonance.sql failed & pause & exit /b 1 )
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\spell_dbc_vanishing_backstab.sql"
if %ERRORLEVEL% neq 0 ( echo ERROR: spell_dbc_vanishing_backstab.sql failed & pause & exit /b 1 )
%MYSQL% -u %USER% -p%PASS% acore_world < "%SQL_WORLD%\spell_dbc_arcane_shot_variants.sql"
...
echo [4/5] Patching SpellItemEnchantment.dbc and rebuilding patch-4.MPQ...
...
echo [5/5] Patching Spell.dbc / SkillLineAbility.dbc and rebuilding patch-z.MPQ...
